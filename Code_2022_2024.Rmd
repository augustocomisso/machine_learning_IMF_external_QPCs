...
```{r}
result <- base %>%
  filter(Arrangement.Number %in% c(826, 833, 842, 845, 847, 848, 855, 858, 862, 872, 883)) %>%
...
## Test solo principales deudores (GRA-5)
...
```{r}
validation_2013 <- base %>%
  filter(Arrangement.Number %in% train_final) %>%  
  filter(Arrangement.Number >= number3) %>%
  pull(Arrangement.Number)

validation_2013 <- sort(unique(validation_2013))
sort(validation_2013)
```

```{r}
length(validation_2013)
```

```{r}
result <- base %>%
  filter(Arrangement.Number %in% validation_2013) %>%
  distinct(QPC_Test_Date) %>%
  pull(QPC_Test_Date)
...
```{r}
gap <- setdiff(setdiff(train_final, train_inicial_2012_atras),validation_2013)
...
```{r}
validation_prog <- validation_2013
validation_prog
```
...
```{r}
#Ukraine
which(test_set$Country.Code == 926)
```
