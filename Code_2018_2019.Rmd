---
title: "Predicción del
incumplimiento de las
metas cuantitativas de
performance en los programas
del FMI: un enfoque basado
en el aprendizaje
automático."
...
```{r}
library(readxl)
base <- read_excel("C:/Users/HP/Desktop/Tesis UDESA/tabla_para_modelos19.xlsx")
...
```{r}
base <- base %>% filter(QPC_Criteria_Order == 32)
unique(base$QPC_Criteria_Order)
...
```{r}
base <- base %>% filter(GRA_redux == 1)
unique(base$GRA_redux)
unique(base$PRGT_redux)
```
...
## Solo QPCs de 2019 para atras

```{r}
base <- base %>% filter(year(QPC_Test_Date) != 2025)
base <- base %>% filter(year(QPC_Test_Date) != 2024)
base <- base %>% filter(year(QPC_Test_Date) != 2023)
base <- base %>% filter(year(QPC_Test_Date) != 2022)
base <- base %>% filter(year(QPC_Test_Date) != 2021)
base <- base %>% filter(year(QPC_Test_Date) != 2020)
```
...
```{r}
base <- base %>% select(-QPC_Criteria_Order)
```
...
## Test GRA (Programa 770 marca el inicio del test)

```{r}
result <- base %>%
  filter(Arrangement.Number == 770) %>%
...
```{r}
result <- base %>%
  filter(Arrangement.Number %in% c(770, 772, 774, 777, 778, 781)) %>%
...
## Test solo principales deudores GRA
...
sort(unique(subset(base, 
                   Country.Code == 213 & 
                   year(QPC_Test_Date) <= 2019 & 
                   year(QPC_Test_Date) >= 2018)$Arrangement.Number))
```

```{r}
result <- base %>%
  filter(Arrangement.Number %in% c(770)) %>%
  select(Arrangement.Number, Arrangement.Type) %>%
  distinct()
...
```{r}
result <- base %>%
  filter(Arrangement.Number %in% c(752)) %>%
  select(Arrangement.Number, Arrangement.Type) %>%
  distinct()
...
```{r}
result <- base %>%
  filter(Arrangement.Number %in% c(777)) %>%
  select(Arrangement.Number, Arrangement.Type) %>%
  distinct()
...
```{r}
prog_test_5gra <- c(777, #Ecuador
                    781, #Pakistan
                    770 #Arg 
                    #752  #Egypt (arranca en 2016, lo excluimos)
                   )
...
programas_2017_o_menos <- base %>%
  filter(year(QPC_Test_Date) <= 2017) %>%
  distinct(Arrangement.Number) %>%
pull(Arrangement.Number)

# Filtrar programas desde 2018 en adelante
programas_2018_o_mas <- base %>%
  filter(year(QPC_Test_Date) >= 2018) %>%
  distinct(Arrangement.Number) %>%
  pull(Arrangement.Number)

# Obtener los que están en 2017 o antes y NO en 2018 o después
train_final <- setdiff(programas_2017_o_menos, programas_2018_o_mas)
...
validation_ <- sort(unique(validation_))
sort(validation_)
```
...
```{r}
result <- base %>%
  filter(Arrangement.Number %in% validation_) %>%
  distinct(QPC_Test_Date) %>%
  pull(QPC_Test_Date)
...
```{r}
# Filtrar programas hasta 2009
programas_2012_o_menos <- base %>%
  filter(year(QPC_Test_Date) < year_val) %>%
  distinct(Arrangement.Number) %>%
  pull(Arrangement.Number)

# Filtrar programas desde 2010 en adelante
programas_2013_o_mas <- base %>%
  filter(year(QPC_Test_Date) >= year_val) %>%
  distinct(Arrangement.Number) %>%
  pull(Arrangement.Number)

# Obtener los que están en 2012 o antes y NO en 2013 o después
train_inicial_2012_atras <- setdiff(programas_2012_o_menos, programas_2013_o_mas)

train_inicial_2012_atras <- sort(train_inicial_2012_atras)
sort(train_inicial_2012_atras)
```

```{r}
length(train_inicial_2012_atras)
```

```{r}
result <- base %>%
  filter(Arrangement.Number %in% train_inicial_2012_atras) %>%
  distinct(QPC_Test_Date) %>%
  pull(QPC_Test_Date)
...
```{r}
gap <- setdiff(setdiff(train_final, train_inicial_2012_atras),validation_)
...
base <- base %>% select(-QPC_Amount_span)
#base <- base %>% select(-Revised.Amount)
#base <- base %>% select(-QPC_Adjusted_Amount_real)
base <- base %>% select(-QPC_Actual_Amount_real)
base <- base %>% select(-Country_1)
base <- base %>% select(-Country_2)
#base <- base %>% select(-Purchase_date_span)
...
```{r}
training_prog <- train_inicial_2012_atras
training_prog
```

```{r}
validation_prog <- validation_
validation_prog
```
...
training_set2 <- base %>% dplyr::filter(Arrangement.Number %in% training_prog2)
...
test_set2 <- base %>% dplyr::filter(Arrangement.Number %in% test_prog2)
...
test_set2 <- test_set2 %>% select(-Arrangement.Number)
...
```{r}
#Test set
test_table2 <- table(test_set2$Status)
sum(test_table2)
test_table2
...
grid <- expand.grid(
  ntree = c(500,1000),
  mtry = unique(pmax(1, c(round(sqrt(p)), round(p*0.1), round(p*0.3), round(p*0.6)))),
  nodesize = c(1, 2),
  maxnodes = c(NA, 20)  # NA = sin límite
)
```
...
class_w <- as.numeric(2*(freq)/sum(freq))
class_w
...
# Performance en test set 3-GRA (out of sample)
...
```{r}
predicted_total_enteros=rep(NA,dim(test_set)[1]) 
...
confmatrix <- table(test_set$Status, predicted_total_enteros)
...
test_set$PredictedStatus <- predicted_total_enteros
...
table_df <- data.frame(
  ...
  actual_status = test_set$Status,
  Country = test_set$Country.Code
)
...
```{r}
predicted_total_enteros_best=rep(NA,dim(test_set)[1]) 
...
confmatrix <- table(test_set$Status, predicted_total_enteros_best)
rownames(confmatrix) <- paste("Real", rownames(confmatrix), sep = ":")
colnames(confmatrix) <- paste("Pred", colnames(confmatrix), sep = ":")
print(confmatrix)
...
library(pROC)
predictions_train <- predict(model1, newdata = xval, type = "prob")
predictions_test <- predict(model2, newdata = test_numeric, type = "prob")
...
ggplot(roc_data, aes(x = 1 - FPR, y = TPR, color = Model, linetype = Model)) +
  ...
  theme_minimal(base_family = "sans", base_size = 11) +
  theme(
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, family = "sans", size = 16),
...
shap_values <- treeshap(rf_unified, test_set[, -1])  # Excluir la variable objetivo
...
# Obtener importancia para filtrar top 15
feature_imp <- aggregate(abs(shap_value) ~ feature, shap_long, mean)
feature_imp <- feature_imp[order(-feature_imp$`abs(shap_value)`), ]

# Obtener top 15 características y excluir la primera
top_15_features <- head(feature_imp$feature, 15)
top_14_features <- top_15_features[-1]  # Excluir la primera característica

# Filtrar solo top 14 (sin la primera)
shap_top14 <- shap_long[shap_long$feature %in% top_14_features, ]

# Crear beeswarm plot
ggplot(shap_top14, aes(x = shap_value, y = reorder(feature, abs(shap_value)), 
                      color = shap_value)) +
  geom_point(alpha = 0.6, size = 1.5) +
  scale_color_gradient2(low = "#A83232", mid = "lightgrey", high = "#3A5F7F", 
                       midpoint = 0, name = "SHAP Value") +
...
```{r}
which(test_set$Country.Code == 213)
...
```{r}
predicted_total_enteros=rep(NA,dim(test_set2)[1]) 
...
confmatrix <- table(test_set2$Status, predicted_total_enteros)
rownames(confmatrix) <- paste("Real", rownames(confmatrix), sep = ":")
colnames(confmatrix) <- paste("Pred", colnames(confmatrix), sep = ":")
print(confmatrix)
...
sum_access <- sum(test_set2$Totalaccess)

test_set2 <- test_set2 %>%
  mutate(w = Totalaccess / sum_access)

test_set2$PredictedStatus <- predicted_total_enteros
...
weighted_TP <- sum(test_set2$w * test_set2$TP)
weighted_FP <- sum(test_set2$w * test_set2$FP)
weighted_FN <- sum(test_set2$w * test_set2$FN)

weighted_F1 <- (2 * weighted_TP) / (2 * weighted_TP + weighted_FP + weighted_FN)

cat(sprintf("Weighted F1 Score: %.7f\n", weighted_F1))

test_set2 <- test_set2 %>% select(-w)
test_set2 <- test_set2 %>% select(-TP)
test_set2 <- test_set2 %>% select(-FP)
test_set2 <- test_set2 %>% select(-FN)
test_set2 <- test_set2 %>% select(-PredictedStatus)
```
...
table_df <- data.frame(
  ...
  prob_1 = predict_vote_model2[,2],
  predicted_enteros = predicted_total_enteros,
  actual_status = test_set2$Status,
  Country = test_set2$Country.Code
)
...
for (i in seq_along(thresholds)) {
  ...
  cat("Longitud Status:", length(test_set2$Status), "\n")
  cat("Longitud predicciones:", length(predicted_entero_loop), "\n")
  

  confmatrix <- table(factor(test_set2$Status, levels = c(0,1)), 
                      factor(predicted_entero_loop, levels = c(0,1)))
...
```{r}
predicted_total_enteros_best=rep(NA,dim(test_set2)[1]) 
...
confmatrix <- table(test_set2$Status, predicted_total_enteros_best)
rownames(confmatrix) <- paste("Real", rownames(confmatrix), sep = ":")
colnames(confmatrix) <- paste("Pred", colnames(confmatrix), sep = ":")
print(confmatrix)
